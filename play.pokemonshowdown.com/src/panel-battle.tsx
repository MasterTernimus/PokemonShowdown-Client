...
class BattlePanel extends PSRoomPanel<BattleRoom> {
	...
	toggleBoostedMove = (e: Event) => {
		...
		case 'mega':
			...
			if (checkbox.checked) choices.current.megax = choices.current.megay = false;
			break;
		case 'megax':
			choices.current.megax = checkbox.checked;
			if (checkbox.checked) choices.current.mega = choices.current.megay = false;
			break;
		case 'megay':
			choices.current.megay = checkbox.checked;
			if (checkbox.checked) choices.current.mega = choices.current.megax = false;
			break;
		case 'ultra':
	...
	renderPlayerControls() {
		...
		case 'move': {
			...
			if (toID(pokemon.item) === 'gardevoirite' && (isGardevoirMega || ['gardevoir', 'gardevoirvoid'].includes(speciesid))) {
				if (['gardevoir', 'gardevoirvoid'].includes(speciesid)) {
